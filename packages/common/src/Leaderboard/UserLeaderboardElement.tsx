...
export const UserLeaderboardElement = React.memo(({
  ...
  const { loading, image, metadata, error } = useTokenMetadata(tokenRef?.mint)
  const name = metadata?.data.name;
  const symbol = metadata?.data.symbol;
  const { handle: reverseTwitterHandle, error: reverseTwitterError } = useReverseTwitter(tokenRef?.owner as PublicKey | undefined);
