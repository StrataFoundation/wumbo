...
import { amountAsNum, Spinner, supplyAsNum, useBondingPricing, useErrorHandler, useMint, useTokenAccount, useTokenBonding } from "@strata-foundation/react";
import React from "react";
import { AiFillCaretDown, AiFillCaretUp } from "react-icons/ai";
import { useTokenBondingRecentTransactions } from "./contexts";

export const PriceChangeTicker = ({ tokenBonding }: { tokenBonding: PublicKey }) => {
  const { transactions, loading, error, hasMore } = useTokenBondingRecentTransactions();
  const { pricing, loading: loadingPricing, error: pricingError } = useBondingPricing(tokenBonding);
  ...
  if (loading || loadingPricing || hasMore || loadingBonding || !baseMint || !baseStorage) {
    return <Box>
      <Spinner size="xs" />
    </Box>
  }
  ...
  const currentPrice = pricingCurve.current(R - totalBaseMintChange, S - totalTargetMintChange)
  ...
  >
    {currentPrice < prevPrice ? <Icon as={AiFillCaretDown} /> : <Icon as={AiFillCaretUp} />}
    <Text
      fontSize="sm"
    >
