...
export const TokenBonding: TypedAccountParser<ITokenBonding> = (
  pubkey,
  account
) => {
  const coded = tokenBondingCoder.accounts.decode<TokenBondingV0>(
    "TokenBondingV0",
    account.data
  );
...
export const Curve: TypedAccountParser<ICurve> = (pubkey, account) => {
  const coded = tokenBondingCoder.accounts.decode<CurveV0>(
    "CurveV0",
    account.data
  );
