...
export async function getTokenRefKeyFromOwner(
  owner: PublicKey
): Promise<PublicKey> {
  return (
...
export async function getTwitterClaimedTokenRefKey(
  ...
  const owner = (await getTwitterRegistry(connection, handle, await getTld()))
    .owner;
...
export async function getReverseTokenRefKey(
...
export const useClaimedTwitterTokenRefKey = (
  ...
  const connection = useConnection();
  const { result: key, loading } = useAsync(
    async (connection: Connection | undefined, name: string | undefined) => {
      if (connection && name) {
...
export const useClaimedTokenRefKey = (
  ...
  const { result } = useAsync(
    async (owner: PublicKey | undefined) =>
      owner && getTokenRefKeyFromOwner(owner),
    [owner]
  );
...
export const useTwitterTokenRef = (
  ...
  const { result: claimedKey, loading: twitterLoading } =
    useClaimedTwitterTokenRefKey(name);
  const { result: unclaimedKey, loading: claimedLoading } =
    useUnclaimedTwitterTokenRefKey(name);
