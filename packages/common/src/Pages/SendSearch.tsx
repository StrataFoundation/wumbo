...
import { StackDivider, Icon, VStack, Text, Input, InputGroup, InputLeftElement, Center } from "@chakra-ui/react";
import { RiCoinLine } from "react-icons/ri";
import Fuse from "fuse.js";
import { useHistory } from "react-router-dom";

const SearchError = ({ title = "", subTitle = "", description = "" }: { title: string, subTitle: string, description: string }) => {
  return <VStack px={8} py={4} rounded={4} spacing={0} border="1px solid #E1E3E8">
    <Icon h="44px" w="44px" as={RiCoinLine} color="gray.300" />
    <Text fontWeight={800} fontSize="14px">{title}</Text>
    <Text fontSize="14px">{subTitle}</Text>
    <Text mt={4} fontSize="14px" color="gray.500">{description}</Text>
  </VStack>
}

export const SendSearch = React.memo(({ getSendLink }: { getSendLink: (tokenWithMeta: ITokenWithMetaAndAccount) => string }) => {
  ...
  const searched = useMemo(() => {
    if (tokens) {
      const sorted = tokens?.filter(t => !!t.tokenRef && t.tokenRef.wumbo.equals(WUMBO_INSTANCE_KEY))
  ...
  const tokenInfos = searched
    .map((tokenWithMeta, index) =>
      <TokenInfo
