import { Text, HStack, VStack, Center, Box, FormLabel, Input, FormControl, InputRightElement, InputGroup, InputLeftElement, Circle, Textarea, Button, Alert, AlertIcon, Icon, Flex, ScaleFade } from '@chakra-ui/react';
...
import { getImage, useAccount, useAssociatedAccount, useAssociatedTokenAddress, useClaimedTokenRef, useFiatPrice, useMint, useOwnedAmount, useSolPrice, useTokenMetadata } from '../utils';
...
export const Send = ({
  ...
  const validationSchema = yup.object({
    amount: yup.number().required().max(ownedAmount || 0).positive(),
  ...
  const { result: image, error: imageError, loading: imageLoading } = useAsync(getImage, [metadata?.data.uri])
  ...
  const handleOnSubmit = async (values: FormValues) => {
    ...
    await provider!.send(tx, [], { commitment: "confirmed", preflightCommitment: "confirmed" });
    ...
    toast.custom((t) => (
      <Notification
        ...
        message={`Succesfully sent ${Number(values.amount).toFixed(
          4
        )} of ${baseMetadata?.data.symbol}!`}
  ...
  return <form onSubmit={handleSubmit(handleOnSubmit)}>
    <VStack spacing={4}>
      ...
      <FormControl>
        ...
        <InputGroup>
          <Input autoFocus size="lg" type="number" {...register("amount")} max={ownedAmount} step={0.0000000001} />
          <InputRightElement h="100%" width="inherit" maxWidth="80px" minWidth="40px">
            <Text color="gray.700">{baseMetadata?.data.symbol}</Text>
          </InputRightElement>
        </InputGroup>
      </FormControl>
      {errors.amount && <Alert status="error" rounded={4}>
        <AlertIcon />
      ...
      <FormControl>
        ...
        <HStack spacing={4} rounded={4} border="1px" borderColor="gray.200" p={4}>
          <Center>
            ...
            >
              { recipient && !ataLoading && <Center>
                { (noAta || noBalance) ? 
                  <Icon color="#dd6b20" as={AiOutlineExclamation}/> :
                  <Icon color="green" as={BiCheck}/>
                }
              </Center> }
              { invalidAddress && <Center>
                <Icon color="red" as={AiOutlineExclamation}/>
              </Center>}
            </Circle>}
          </Center>
          <VStack w="full" spacing={1} alignItems="start" justifyContent="stretch">
            {metadata && <Text flexGrow={0} fontWeight={800}>{metadata.data.name}</Text>}
            {recipient && noBalance && <Text flexGrow={0} fontSize="xs" color="#dd6b20">This address does not own any {baseMetadata?.data.symbol}</Text>}
            {recipient && noAta && <Text flexGrow={0} fontSize="xs" color="#dd6b20">This address has no {baseMetadata?.data.symbol} account, you will pay to create it</Text>}
            {invalidAddress && <Text flexGrow={0} fontSize="xs" color="red">Invalid address</Text>}
            <Text
              {...recipientRegister}
              onInput={(e) => {
                // @ts-ignore
      ...
      {errors.recipient && <Alert rounded={4} status="error">
        <AlertIcon />
      ...
      >
        <Flex justify="space-between" alignItems="center">
          ...
          <Text>
            {amount} {baseMetadata?.data.symbol}{ amount && fiatPrice ? ` ≈ $${toFiat(amount)?.toFixed(2)}` : ""}
          </Text>
        </Flex>
        <Flex justify="space-between" alignItems="center">
          <Text>Estimated Fees</Text>
          <Flex>{fee} SOL ≈ ${solFiatPrice && fee ? (solFiatPrice * fee).toFixed(2) : undefined}</Flex>
        </Flex>
      </VStack>
      <Button
        isDisabled={!!errors.amount || !!errors.recipient || invalidAddress || !recipientStr}
        w="full"
        type="submit"
        colorScheme="indigo"
        size="lg"
        isLoading={isSubmitting || myAtaLoading || ataLoading || recipientLoading || awaitingApproval}
        loadingText={
          awaitingApproval
            ? "Awaiting Approval"
            : isSubmitting ? "Sending" : "Loading"
        }
      >
        Send
      </Button>
