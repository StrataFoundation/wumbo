...
import { SplTokenAccountSplit, SplTokenAccountSplitIDL } from "@wum.bo/spl-token-account-split";
...
export async function getPrograms(provider?: anchor.Provider): Promise<AnchorPrograms> {
  if (!provider) return {};

  const SplTokenBondingIDLJson = await anchor.Program.fetchIdl(splTokenBondingProgramId, provider);
  const SplWumboIDLJson = await anchor.Program.fetchIdl(splWumboProgramId, provider);
