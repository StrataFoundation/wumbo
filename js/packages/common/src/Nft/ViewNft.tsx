...
export const ViewNftRaw = React.memo(({ token, getCreatorLink }: { token: ITokenWithMeta, getCreatorLink: GetCreatorLink }) => {
  ...
  return <div className="grid grid-cols-2 sm:grid-cols-1">
    ...
    <div className="p-2 flex flex-col space-y-2">
      <span className="text-2xl font-extrabold block">{token.metadata?.data.name}</span>
          ...
          <div className="pb-4">
            <div className="space-y-2 pb-4">
              <span className="font-medium">
                {displayName(token.data?.properties.category)} • {token.edition ? `Edition no. ${token.edition.edition.toNumber()} of ${token.masterEdition?.supply.toNumber()}` : 'Master Edition'}
              </span>
              {token.metadata && <div className="font-medium flex flex-row">
                <span>Creators:&nbsp;</span>
                {token.metadata?.data.creators?.filter(c => c.verified).map(creator =>
                  <Creator key={creator.address.toBase58()} creator={creator.address} getCreatorLink={getCreatorLink} />
                )}
                <span>•</span>
                <span>Authority:&nbsp;</span>
                {<Creator creator={token.metadata.updateAuthority} getCreatorLink={getCreatorLink} />}
              </div>}
            </div>
