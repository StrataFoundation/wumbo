...
import {SPL_ASSOCIATED_TOKEN_ACCOUNT_PROGRAM_ID, TOKEN_PROGRAM_ID} from "../globals";
import {TokenAccountParser} from "@oyster/common/lib/contexts/accounts";


const fetch = (wallet: PublicKey, mint: PublicKey) => Token.getAssociatedTokenAddress(
...
export function useAssociatedTokenAddress(wallet: PublicKey, mint: PublicKey): AssocState{
...
export function useAssociatedAccount(wallet: PublicKey, mint: PublicKey): AssociatedAccountState {
  const { result: associatedTokenAddress, loading: associatedTokenLoading } = useAssociatedTokenAddress(wallet, mint)
  const { account, loading } = useReactiveAccount(associatedTokenAddress);
  const [state, setState] = useState<TokenAccountInfo>()

  useEffect(() => {
    associatedTokenAddress && account && setState(TokenAccountParser(associatedTokenAddress, account).info)
