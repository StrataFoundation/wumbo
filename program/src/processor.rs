...
fn process_initialize_solclout(
    ...
    let (solclout_key, bump) = Pubkey::find_program_address(&[&solclout_storage.mint.to_bytes()[..32]], program_id);
    if solclout_key != *solclout.key {
        return Err(SolcloutError::InvalidProgramAddress.into());
    }

    if solclout.data.borrow().len() > 0 && SolcloutInstance::unpack_from_slice(&solclout.data.borrow())?.initialized {
...
mod tests {
    ...
    fn test_initialize_solclout() {
        ...
        let (instance_key, _) = Pubkey::find_program_address(&[&mint_key.to_bytes()[..32]], &program_id);
    ...
    fn test_buy() {
        ...
        let (solclout_authority_key, solclout_authority_nonce) = Pubkey::find_program_address(&[&solclout_instance_key.to_bytes()[..32]], &program_id);
        ...
        let (founder_rewards_account_key, mut founder_rewards_account) = get_account(Account::LEN, &token_program_id);
        initialize_spl_account(&mut founder_rewards_account, &token_program_id, &solclout_mint_key, &solclout_authority_key);
